...
async def request_otp(user: UserSignUp):
...
async def verify_otp(user: UserSignUp):
    if not user.otp:
        raise HTTPException(status_code=401, detail="OTP is required")
...
async def sign_in_password(user: UserSignIn):
